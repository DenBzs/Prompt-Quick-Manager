    ...
    <div class="ptm-card" data-gi="${gi}">
        <div class="ptm-card-head">
            ...
            <div class="ptm-gbtns">
                ...
                ${!groupReorderMode && !inToggleReorder ? `<button class="ptm-ibtn ptm-popup-pin ptm-pin-btn${g.showInPopup ? ' ptm-pin-on' : ' ptm-pin-off'}" data-gi="${gi}" title="미니창에 표시">📌</button>` : ''}
...
function wireGroupCards(area) {
    ...
    // 📌 팝업 핀 버튼
    area.querySelectorAll('.ptm-pin-grp').forEach(btn => btn.addEventListener('click', () => {
        const gi = +btn.dataset.gi, pn = getCurrentPreset(), gs = getGroupsForPreset(pn);
...
function buildPpcSubHtml(gi) {
    ...
    const rows = g.toggles.map((t, ti) => {
        ...
        <div style="display:flex;align-items:center;gap:5px;padding:5px 0;border-bottom:1px solid rgba(0,0,0,0.08);">
            <span style="font-size:11px;min-width:22px;text-align:center;font-weight:700;color:${effectOn ? '#5ab882' : '#999'};">${effectOn ? 'On' : 'Off'}</span>
            <button class="ppc-sub-ovr" data-ti="${ti}"
                style="border:none;border-radius:3px;padding:2px 5px;font-size:10px;font-weight:600;cursor:pointer;flex-shrink:0;background:${ovrBg};color:${ovrClr};">
                ${ovrLabel}
            </button>
            ...
            <button class="ppc-sub-bsel" data-ti="${ti}"
                style="border:none;border-radius:3px;padding:2px 5px;font-size:10px;font-weight:600;cursor:pointer;flex-shrink:0;background:${bBg};color:${bClr};">
                ${isDirect ? '동일' : '반전'}
            </button>
