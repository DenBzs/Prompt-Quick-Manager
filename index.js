...
function buildGroupCard(g, gi, pn) {
    ...
    <div class="ptm-card" data-gi="${gi}">
        ...
        <label class="ptm-popup-chk-label">
            <input type="checkbox" class="ptm-popup-chk" data-gi="${gi}" ${g.showInPopup ? 'checked' : ''}>
            <span>팝업에 표시</span>
        </label>` : ''}
...
function wireGroupCards(area) {
    ...
    // "팝업에 표시" checkbox
    area.querySelectorAll('.ptm-popup-chk').forEach(cb => cb.addEventListener('change', () => {
...
function renderPpcLower() {
    ...
    // Wire header toggle
    lower.querySelector('#ppc-grp-head').addEventListener('click', () => {
...
function buildPpcSubHtml(gi) {
    ...
    const grpBg  = g.isOn ? '#6ddb9e' : '#aaa';
    const grpClr = g.isOn ? '#1a1a1a' : '#eee';
    ...
    const rows = g.toggles.map((t, ti) => {
        ...
        <div style="display:flex;align-items:center;gap:5px;padding:5px 0;border-bottom:1px solid rgba(0,0,0,0.08);">
            ...
            <button class="ppc-sub-ovr" data-ti="${ti}"
                style="border:none;border-radius:3px;padding:2px 7px;font-size:11px;cursor:pointer;flex-shrink:0;background:${ovrBg};color:${ovrClr};">
                ${ovrLabel}
            </button>
