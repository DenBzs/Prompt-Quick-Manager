...
function wireGroupCards(area) {
    ...
    area.querySelectorAll('label.ptm-popup-pin').forEach(lbl => lbl.addEventListener('click', e => {
        e.stopPropagation();
    }));
}
    ...
    if (ppcGroupsExpanded) {
        if (!visible.length) {
            ...
            rowsHtml = visible.map(({ g, gi }) => {
                const bg  = g.isOn ? '#6ddb9e' : '#bbb';
                const clr = g.isOn ? '#1a1a1a' : '#555';
                return `
...
function positionPpcSub(sub) {
    ...
    // Center horizontally, aligned with popup
    let left;
    if (popup) {
    ...
    // Always above the popup — never overlap it
    let top;
    if (popup) {
        const pr = popup.getBoundingClientRect();
        top = pr.top - subH - 10;
        if (top < 8) {
            // Not enough room above — push popup down instead by centering vertically above the fold
            top = 8;
        }
    } else {
...
function buildPpcSubHtml(gi) {
    ...
    const grpBg  = g.isOn ? '#5ab882' : '#b85a5a';
    const grpClr = '#fff';
    ...
    const rows = g.toggles.map((t, ti) => {
        ...
        if (ovr === null)      { ovrLabel = '고정'; ovrBg = 'rgba(150,150,150,0.3)'; ovrClr = '#ccc'; }
        else if (ovr === true) { ovrLabel = 'On';  ovrBg = 'rgba(90,184,130,0.3)';  ovrClr = '#5ab882'; }
        else                   { ovrLabel = 'Off'; ovrBg = 'rgba(184,90,90,0.3)';   ovrClr = '#c07070'; }

        const bBg  = isDirect ? 'rgba(150,150,150,0.3)' : 'rgba(122,100,220,0.3)';
        const bClr = isDirect ? '#aaa' : '#a090e8';

        const btnStyle = 'border:none;border-radius:3px;width:30px;min-width:30px;height:18px;font-size:10px;font-weight:700;cursor:pointer;flex-shrink:0;display:inline-flex;align-items:center;justify-content:center;box-sizing:border-box;';
        return `
    ...
    <div style="padding:12px 14px;">
        ...
        <button class="ppc-sub-add"
            style="margin-top:10px;width:100%;border:1px dashed #b0a898;background:transparent;border-radius:5px;padding:6px 0;cursor:pointer;color:#5a5450;font-size:12px;text-align:center;box-sizing:border-box;display:block;">
            + 토글 추가
        </button>
    </div>`;
}
