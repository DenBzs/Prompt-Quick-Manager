...
const PPC_THEMES = {
    dark: {
        label: '🌙',
        title: '다크',
        popup:  { border:'#3a3a4a', upper:'#1e1e2e', lower:'#181826', text:'#d8d4f0', shadow:'0 4px 20px rgba(0,0,0,0.5)' },
        sub:    { bg:'#1e1e2e', border:'#3a3a4a', text:'#d8d4f0' },
        onBg:'#4a8c68', offBg:'#8c4a4a', onClr:'#fff', offClr:'#fff',
        rowBorder:'rgba(255,255,255,0.07)',
    },
    lavender: {
        label: '💜',
        title: '라벤더',
        popup:  { border:'#c8bfe8', upper:'#f4f0fc', lower:'#ede7f8', text:'#3a2e5a', shadow:'0 4px 16px rgba(120,90,200,0.15)' },
        sub:    { bg:'#f4f0fc', border:'#c8bfe8', text:'#3a2e5a' },
        onBg:'#7c5cbf', offBg:'#b05070', onClr:'#fff', offClr:'#fff',
        rowBorder:'rgba(100,80,160,0.12)',
    },
    sky: {
        label: '🩵',
        title: '스카이',
        popup:  { border:'#b8d8f0', upper:'#f0f8ff', lower:'#e4f2fc', text:'#1a3a5a', shadow:'0 4px 16px rgba(60,130,200,0.15)' },
        sub:    { bg:'#f0f8ff', border:'#b8d8f0', text:'#1a3a5a' },
        onBg:'#3a8abf', offBg:'#b05060', onClr:'#fff', offClr:'#fff',
        rowBorder:'rgba(60,130,200,0.12)',
    },
    pink: {
        label: '🌸',
        title: '핑크',
        popup:  { border:'#f0c8d8', upper:'#fff4f8', lower:'#fce8f0', text:'#4a2035', shadow:'0 4px 16px rgba(200,80,120,0.12)' },
        sub:    { bg:'#fff4f8', border:'#f0c8d8', text:'#4a2035' },
        onBg:'#c05880', offBg:'#a04848', onClr:'#fff', offClr:'#fff',
        rowBorder:'rgba(200,80,120,0.12)',
    },
    classic: {
        label: '☕',
        title: '클래식',
        popup:  { border:'#d6cfc3', upper:'#f5f0e8', lower:'#e8e2d8', text:'#2a2a2a', shadow:'0 4px 16px rgba(0,0,0,0.18)' },
        sub:    { bg:'#f5f0e8', border:'#d6cfc3', text:'#2a2a2a' },
        onBg:'#4fa872', offBg:'#a84f4f', onClr:'#fff', offClr:'#fff',
...
function applyPpcTheme() {
    ...
    if (popup) {
        popup.style.border        = `1px solid ${t.popup.border}`;
    ...
    if (sub) {
        ...
        sub.style.border     = `1px solid ${t.sub.border}`;
    ...
    <div class="ptm-card" data-gi="${gi}">
        <div class="ptm-card-head">
            ...
            <div class="ptm-gbtns">
                ...
                ${!groupReorderMode && !inToggleReorder ? `<label class="ptm-ibtn ptm-popup-pin" data-gi="${gi}" title="미니창에 표시" style="cursor:pointer;gap:3px;${g.showInPopup ? 'color:var(--SmartThemeBodyColor,#ddd);opacity:1;' : 'opacity:0.4;'}"><input type="checkbox" class="ptm-popup-chk-input" data-gi="${gi}" ${g.showInPopup ? 'checked' : ''} style="width:11px;height:11px;accent-color:#a090e8;cursor:pointer;margin:0;">팝업</label>` : ''}
...
function wireGroupCards(area) {
    ...
    // 팝업 체크박스
    area.querySelectorAll('.ptm-popup-chk-input').forEach(cb => cb.addEventListener('change', e => {
        e.stopPropagation();
        const gi = +cb.dataset.gi, pn = getCurrentPreset(), gs = getGroupsForPreset(pn);
        gs[gi].showInPopup = cb.checked;
        saveGroups(pn, gs);
        refreshPpcPopup();
        renderTGGroups();
    }));
    area.querySelectorAll('label.ptm-popup-pin').forEach(lbl => {
        lbl.addEventListener('click', e => e.stopPropagation());
        // Prevent the label's own click from double-firing the checkbox
...
function wireTGReorder() {
    ...
    let dragSrc = null;   // { gi, ti, el }
    let dragOver = null;  // target ti
    let ghostEl = null;

    function getDragRow(el) {
        return el?.closest?.('.ptm-trow[data-draggable="true"]') || null;
    }
    ...
    function commitDrop(toTi) {
        if (!dragSrc || toTi == null || toTi === dragSrc.ti) return;
        const pn = getCurrentPreset(), gs = getGroupsForPreset(pn);
        const toggles = gs[dragSrc.gi].toggles;
        const [moved] = toggles.splice(dragSrc.ti, 1);
    ...
    function endDrag(commit) {
        if (!dragSrc) return;
        const toTi = commit ? dragOver : null;
        clearGhost();
        highlightOver(null);
        ...
        dragSrc = null;
        dragOver = null;
        if (commit && toTi != null) commitDrop(toTi);
    ...
    area.addEventListener('touchcancel', () => endDrag(false));
}
...
function getOrCreatePpcPopup() {
    ...
    popup.innerHTML = `
        ...
        <div id="ppc-theme-bar" style="background:#e8e2d8;padding:4px 14px;display:flex;gap:4px;align-items:center;border-top:1px solid rgba(0,0,0,0.06);">
            ${Object.entries(PPC_THEMES).map(([k,t]) =>
                `<button class="ppc-theme-btn" data-theme="${k}"
                    title="${t.title}"
                    style="border:none;background:none;cursor:pointer;font-size:14px;padding:1px 3px;border-radius:4px;line-height:1;opacity:0.5;">
                    ${t.label}
                </button>`
            ).join('')}
        </div>
    ...
    if (ppcGroupsExpanded) {
        if (!visible.length) {
            ...
            rowsHtml = visible.map(({ g, gi }) => {
                const th  = PPC_THEMES[getPpcTheme()] || PPC_THEMES.classic;
                const bg  = g.isOn ? th.onBg  : th.offBg;
                const clr = g.isOn ? th.onClr : th.offClr;
                return `
...
function buildPpcSubHtml(gi) {
    ...
    const t      = PPC_THEMES[getPpcTheme()] || PPC_THEMES.classic;
    const grpBg  = g.isOn ? t.onBg  : t.offBg;
    const grpClr = g.isOn ? t.onClr : t.offClr;
    ...
    const rows = g.toggles.map((t, ti) => {
        ...
        const btnStyle = 'border:none;border-radius:3px;width:34px;min-width:34px;height:18px;font-size:10px;font-weight:700;cursor:pointer;flex-shrink:0;display:inline-flex;align-items:center;justify-content:center;box-sizing:border-box;white-space:nowrap;';
        return `
